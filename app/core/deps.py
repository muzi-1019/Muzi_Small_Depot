...
security = HTTPBearer()  # 创建 Bearer Token 认证依赖，接口中 Depends(security) 会自动校验 Authorization 头格式
...
def get_current_user_id(credentials: HTTPAuthorizationCredentials = Depends(security)) -> int:
    ...
    try:  # JWT 解码可能因为过期、签名错误或格式错误而失败
        ...
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,  # 返回 401 未认证
            detail="Could not validate credentials",  # 提示凭证无效
            headers={"WWW-Authenticate": "Bearer"},  # 告诉客户端需要 Bearer Token
        )  # 抛出 FastAPI HTTP 异常，中断请求
...
def get_chat_service(db: Session = Depends(get_db_session)) -> ChatService:
    ...
    """
    return ChatService(
        character_repository=CharacterRepository(db=db),  # 注入角色仓库
        user_repository=UserRepository(db=db),  # 注入用户仓库
        conversation_repository=ConversationRepository(db=db),  # 注入会话仓库
        memory_service=get_memory_service(),  # 注入对话记忆服务
    )  # 返回完整组装后的聊天服务
