...
class UserRepository:
    ...
    def get_by_account(self, account: str) -> User | None:
        ...
        return self.db.scalar(select(User).where(User.account == account))  # 返回匹配账号的第一条用户记录，查不到则为 None

    def get_by_id(self, user_id: int) -> User | None:
        """根据用户ID查询用户（用于获取用户信息）"""
        return self.db.get(User, user_id)  # 按主键直接查询 User 表，SQLAlchemy 会优先利用会话缓存

    def create_user(self, account: str, password_hash: str, nickname: str = "新用户") -> User:
        """创建新用户并写入数据库（用于注册）"""
        user = User(account=account, password_hash=password_hash, nickname=nickname, preferred_character_ids="[]")  # 构造用户 ORM 对象，默认没有偏好角色
